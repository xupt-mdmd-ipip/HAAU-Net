...
model.load_weights("AAU-net-spatial.h5")

def show_mask_jpg_color(image, mask, cmap=None, alpha=0.4):
    plt.imshow(image)
    plt.imshow(tf.squeeze(mask), cmap=cmap, alpha=alpha)
    plt.axis('off')
    plt.savefig('predict_mask_color.jpg')

def convert_to_black(image):
    grayscale_image = image.convert("L")
    black_image = Image.new("RGB", grayscale_image.size, 0)
    return black_image


def show_mask_jpg(image, mask, cmap=None, alpha=1.0):
    #print(image.shape)
    #print(type(image))
    black_image = convert_to_black(image)
    plt.imshow(black_image)
    #print(tf.squeeze(mask))
    plt.imshow(tf.squeeze(mask), cmap='gray', alpha=alpha)
    plt.axis('off')
...
def predict_mask(image):
    ...
    #img.putdata([0, 0, 0] * (img.size[0] * img.size[1]))
    show_mask_jpg(img,prediction)
    #show_mask_jpg_color(img,prediction)
    mask_jpg = Image.open("predict_mask.jpg")
    #mask_jpg_color = Image.open("predict_mask_color.jpg")
    return mask_jpg


#iface = gr.Interface(predict_mask, inputs="image", outputs=["image","image"])
iface = gr.Interface(predict_mask, inputs="image", outputs="image",title="Spatial-baesed Segementation")
